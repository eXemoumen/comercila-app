...
export default function PendingPaymentsPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        console.log("🔄 Loading virements data...");

        // Use hybrid storage functions instead of direct localStorage
        const allSales = await getSales();
        console.log("📊 Sales loaded:", allSales.length, "records");

        const unpaidSales = allSales.filter((sale) => !sale.isPaid);
        console.log("💰 Unpaid sales:", unpaidSales.length, "records");
        ...
        console.error("Error loading virements data:", error);
        setPendingSales([]);
        setSupermarkets([]);
        setIsLoading(false);
      }
