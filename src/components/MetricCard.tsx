...
export const MetricCard: React.FC<MetricCardProps> = ({
  ...
  return (
    <Card
      className={`overflow-hidden border-none shadow-md rounded-xl hover:shadow-lg transition-all duration-300 ${className}`}
    >
      ...
      <CardContent className="p-3">
        ...
        {additionalInfo && (
          <div
            className={`text-xl font-semibold ${config.text} mt-1 animate-in slide-in-from-bottom-3 duration-300`}
          >
            {additionalInfo}
          </div>
        )}
        ...
        {showProgressBar && percentage !== undefined && (
          <div
            className={`h-3 w-full rounded-full ${config.background} mt-3 overflow-hidden`}
          >
            <div
