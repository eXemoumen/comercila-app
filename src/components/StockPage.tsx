...
import { ChevronLeft, Plus, Minus, Settings } from "lucide-react";
import {
  getStockHistory,
  updateStock,
  getFragranceStock,
  ...
  const loadStockData = useCallback(async () => {
    ...
    console.log(
      ...
      history.map((h) => ({
        ...
        reason: h.reason,
        fragranceDistribution: h.fragranceDistribution,
      }))
    );
    setStockHistory(history);

    // Get current stock using the same method as dashboard
    const { currentStock, fragranceStock: fragmentStock } =
      await getCurrentStock();
    console.log(
      "Loaded current stock:",
      currentStock,
      "fragment stock:",
      fragmentStock
    );

    // Get fragrance stock details
    let fragStock = await getFragranceStock();
    console.log("Loaded fragrance stock:", fragStock);
    console.log(
      "Fragrance stock details:",
      fragStock.map((f) => `${f.name}: ${f.quantity}`)
    );
    ...
    // Use the currentStock from getCurrentStock (this is in cartons)
  ...
  const handleAdjustStock = async (e: React.FormEvent): Promise<void> => {
    ...
    if (isAddingMode) {
      ...
      if (showFragranceForm) {
        ...
        console.log(
          "Updating stock with fragrance distribution:",
        ...
        console.log("Updating stock with even distribution:", evenDistribution);
        updateStock(
          amountToAdd,
          "added",
